...
public class Enemy extends Entity{
	...
	public Enemy(int x, int y, int width, int height, BufferedImage sprite) {
		...
		spritesDamaged = new BufferedImage[5];
		for(int i = 0; i < 5; i++) {
			spritesDamaged[i] = Game.spritesheet.getSprite(32 + (i*16), 80, 16, 16);
		}
	...
	public boolean isColliding(int xn, int yn) {
		Rectangle enemyCurrent = new Rectangle(xn + maskx, yn + masky, maskw, maskh);
		for(int i = 0; i < Game.enemies.size(); i++) {
			...
			if(enemyCurrent.intersects(target)) {
				return true;
			}
		}
		return false;
